...
async function waitForServer() {
  ...
  throw new Error("Next.js Phase 10 test server did not become ready in time.");
}
...
try {
  ...
  const result = spawnSync(
    process.execPath,
    [playwrightCli, "test", "tests/e2e-auth/phase10-sales-visit.spec.ts", "--config", "playwright.phase3.config.ts"],
    { cwd: process.cwd(), env: process.env, stdio: "inherit" },
  );
