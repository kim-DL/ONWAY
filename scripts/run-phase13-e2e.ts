...
const nextServer = spawn(process.execPath, [nextCli, "dev", "--webpack", "--hostname", "127.0.0.1", "--port", "3103"], {
  cwd: process.cwd(),
  env: process.env,
  stdio: "inherit",
});
...
async function waitForServer() {
  ...
  throw new Error("Next.js Phase 13 test server did not become ready in time.");
}
...
try {
  ...
  const result = spawnSync(process.execPath, [
    playwrightCli,
    "test",
    "tests/e2e-auth/phase13-directions.spec.ts",
    "--config",
