...
async function waitForServer() {
  ...
  throw new Error("Next.js Phase 18 rehearsal server did not become ready in time.");
}
...
try {
  ...
  const result = spawnSync(
    process.execPath,
    [playwrightCli, "test", "tests/e2e-auth/phase18-pilot-diagnostics.spec.ts", "--config", "playwright.phase3.config.ts"],
    { cwd: process.cwd(), env: process.env, stdio: "inherit" },
  );
