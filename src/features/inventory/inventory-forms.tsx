...
export function InventoryProductEditor({ product, location, onClose, onSaved }: { product: InventoryProduct | null; location: InventoryLocation; onClose: () => void; onSaved: (product: InventoryProduct) => void }) {
  ...
  return <BottomSheet open title={product ? "품목 정보 수정" : "새 품목 등록"} onClose={onClose} dismissible={!busy}>
    <form id={id} onSubmit={submit} className={`${styles.sheet} ${formStyles.productForm}`} aria-busy={busy}>
      ...
      <div className={formStyles.fieldsPair}><label className={styles.field}>제조사<input maxLength={200} value={draft.manufacturer} onChange={(event) => set("manufacturer", event.target.value)} disabled={busy} /></label><label className={styles.field}>원산지<input maxLength={200} value={draft.origin} onChange={(event) => set("origin", event.target.value)} disabled={busy} /></label></div>
      <div className={formStyles.fieldsPair}><label className={styles.field}>규격<input maxLength={200} placeholder="예: 1kg" value={draft.specification} onChange={(event) => set("specification", event.target.value)} disabled={busy} /></label><label className={styles.field}>기준 단위 <span className={styles.required}>필수</span><input required maxLength={20} placeholder="개, 봉, 팩" value={draft.unitLabel} onChange={(event) => set("unitLabel", event.target.value)} disabled={busy || !!product?.hasHistory} /></label></div>
