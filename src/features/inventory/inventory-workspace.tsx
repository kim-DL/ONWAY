...
export function InventoryWorkspace({ session, admin = false }: { session: AuthenticatedSession; admin?: boolean }) {
  ...
  return <section ref={workspaceRef} className={admin ? styles.workspace : `shell-page ${styles.workspace}`} data-admin={admin || undefined} data-write-actions={context?.canWrite || undefined} aria-label="재고 관리">
    ...
    {context?.canWrite && editorOpen ? <InventoryProductEditor product={null} location={location === "all" ? "refrigerated" : location} canCreateManufacturer={context.canWrite} onClose={() => setEditorOpen(false)} onSaved={(product) => { saved(product); setSelectedId(product.productId); }} /> : null}
    ...
    {context?.canWrite ? <BottomSheet open={optionsOpen} title="목록 옵션" onClose={() => setOptionsOpen(false)}><div className={styles.sheet}>
      <section className={styles.lotCard} aria-labelledby="inventory-display-options"><div className={styles.sectionHeading}><h3 id="inventory-display-options">표시 옵션</h3></div><label className={`${styles.toggle} ${styles.row}`}><span>비활성 품목 보기</span><input type="checkbox" checked={showInactive} onChange={(event) => { setShowInactive(event.target.checked); setLimit(60); }} /></label><label className={`${styles.toggle} ${styles.row}`} title={context ? `유통기한 D-${context.settings.urgentDays}일 이내, 기한 지난 상품 포함` : undefined}><span>임박 상품만 보기<small>{context ? `D-${context.settings.urgentDays}일` : "기준 확인 중"}</small></span><input type="checkbox" disabled={inactiveView || !calendarReady || !context} checked={urgentOnly} onChange={(event) => { setUrgentOnly(event.target.checked); setLimit(60); }} /></label></section>
      <section className={styles.lotCard} aria-labelledby="inventory-work-mode"><div className={styles.sectionHeading}><h3 id="inventory-work-mode">작업 모드</h3></div><label className={`${styles.countToggle} ${styles.row}`} data-active={countMode || undefined}><input type="checkbox" role="switch" aria-label="재고조사 모드" checked={countMode} disabled={!calendarReady} onChange={(event) => changeCountMode(event.target.checked)} /><span className={styles.countToggleTrack} aria-hidden="true"><span><Icon name="check" size={11} /></span></span><span className={styles.countToggleLabel} aria-label={countMode ? "재고조사 ON" : "재고조사 OFF"}>재고조사<small className={styles.countToggleState} aria-hidden="true">{countMode ? "on" : "off"}</small></span></label></section>
