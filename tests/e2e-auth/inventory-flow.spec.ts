...
test("PIN user registers a photographed product, receives/counts/issues stock, and replaces/removes its private photo", async ({ page }, info) => {
  ...
  await editor.getByLabel("제조사", { exact: true }).fill("테스트 제조사");
  await editor.getByLabel("규격", { exact: true }).fill("1kg");
  await editor.getByLabel("원산지", { exact: true }).fill("대한민국");
  await editor.getByLabel(/기준 단위/).fill("봉");
  ...
  await expect(edit.getByLabel(/기준 단위/)).toBeDisabled();
...
test("an offline registration draft stays in memory and saves only after an explicit retry online", async ({ page, context }, info) => {
  ...
  await editor.getByRole("radio", { name: "미확인", exact: true }).check();
