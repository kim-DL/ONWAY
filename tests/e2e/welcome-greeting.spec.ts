...
const animatedPath = "/brand/bloub-welcome-v1.webp";
const stillPath = "/brand/bloub-welcome-still-v1.png";
let script = "";
let moduleCss = "";
...
test.beforeAll(async () => {
  ...
  expect(loopDurationMs).toBeGreaterThanOrEqual(15_000);
  expect(loopDurationMs).toBeLessThanOrEqual(16_000);
});
  ...
  await page.route("https://welcome.fixture/**", async route => {
    ...
    await route.fulfill({ contentType: "text/html", body: `<!doctype html><html lang="ko"><head><meta charset="utf-8">
      ...
      <style>${globals}</style><style>${moduleCss}</style><style>
        ...
        #welcome-fixture > h1 { font:700 1.1rem/1.4 sans-serif; margin:0 0 12px; }
...
async function expectReadableLayout(target: Locator) {
  const geometry = await target.evaluate(element => {
    ...
    return {
      // The source WebP's transparent canvas intentionally extends 6px past
      // the slot; measure content and its reserved slot, not that canvas.
      overflows: copy.scrollWidth - copy.clientWidth,
      slotClipped: mascotBox.left < greetingBox.left - 1 || mascotBox.right > greetingBox.right + 1,
      viewportOverflow: document.documentElement.scrollWidth - innerWidth,
      textClipped: lines.some(line => line.left < copyBox.left - 1 || line.right > copyBox.right + 1
        || line.top < copyBox.top - 1 || line.bottom > copyBox.bottom + 1),
      overlaps: lines.some(line => line.left < mascotBox.right - 1 && line.right > mascotBox.left + 1
        && line.top < mascotBox.bottom - 1 && line.bottom > mascotBox.top + 1),
      height: element.getBoundingClientRect().height,
    };
  });
  expect(geometry.overflows).toBeLessThanOrEqual(1);
...
for (const placement of ["delivery", "sales", "activity"] as const) {
  test(`${placement} greeting keeps long names readable at 320/390/1280px and 200% text`, async ({ page }, testInfo) => {
...
test("decorative mascot and shared motion controls meet accessible semantics", async ({ page }) => {
  ...
  await expect(image(page)).toHaveAttribute("alt", "");
  await expect(greeting(page).getByRole("img")).toHaveCount(0);
  const result = await new AxeBuilder({ page }).include("#welcome-fixture").withTags(["wcag2a", "wcag2aa", "wcag21aa"]).analyze();
